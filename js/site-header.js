...
(function(){
  ...
  const logoutButton=previous?.querySelector("#sheetLogout,#adminLogout,#shopLogout,#accountLogout,#charactersLogout,#logoutButton,button[id*='Logout']");
  const adminButton=previous?.querySelector("#adminPanelButton");
  const header=document.createElement("header");header.className="site-header";
  header.innerHTML=`<a class="site-header__brand" href="index.html" aria-label="Ir para a tela inicial"><img src="assets/images/logo.png" alt=""><span>Wonderland</span></a><nav class="site-header__nav" aria-label="Navegação principal">${routes.map(([label,href])=>`<a href="${href}" class="${current===href?"active":""}">${label}</a>`).join("")}</nav><div class="site-header__right"><button class="site-header__action site-header__menu-button" type="button" aria-expanded="false" aria-label="Abrir navegação">☰ Navegação</button><a class="site-header__action" href="menu.html">Menu</a>${adminButton&&!adminButton.hidden?'<a class="site-header__action" href="admin.html">Painel ADM</a>':""}${logoutButton?'<button class="site-header__action site-header__action--danger" id="siteHeaderLogout" type="button">Sair</button>':""}</div>`;
