...
import {
  ...
  type SimpleClassDraft,
  type SimpleItemDraft,
  type SimpleRaceDraft,
...
type Existing = {
  classes: SimpleClassDraft[];
  races: SimpleRaceDraft[];
...
export function AdminContentStudio({
  ...
  function newDraft() {
    setMessage("");
    if (kind === "class") setClassDraft(simpleClassDefaults());
    if (kind === "race") setRaceDraft(simpleRaceDefaults());
  ...
  function loadExisting(id: string) {
    ...
    if (kind === "class") setClassDraft(found as SimpleClassDraft);
    if (kind === "race") setRaceDraft(found as SimpleRaceDraft);
  ...
  function save() {
    ...
    startSaving(async () => {
      const result =
        kind === "class"
          ? await saveSimpleClassAction(classDraft)
          : kind === "race"
            ? await saveSimpleRaceAction(raceDraft)
            : kind === "item"
    ...
    <section className={styles.builder}>
      ...
      <div className={styles.aiSubPanel}>
        ...
        <form action={aiAction} className={styles.aiContentForm}>
          ...
          <textarea
            ...
            rows={3}
            required
            placeholder={`Ex.: Crie ${kind === "class" ? "uma classe de cavaleiro rúnico" : kind === "race" ? "uma raça de gigantes de gelo" : kind === "item" ? "uma espada lendária focada em FOR e sangramento" : "um título para fundadores"}.`}
          />
          <label>
      ...
      <div className={styles.editorLayout}>
        ...
        <aside className={styles.preview}>
          ...
          <p>
            {kind === "class"
              ? classDraft.description
              : kind === "race"
                ? raceDraft.description
                : kind === "item"
                  ? itemDraft.description
                  : titleDraft.description}
          </p>
          <dl>
            <div>
              <dt>Tipo</dt>
            ...
            {kind === "class" ? (
              <>
                <div>
                  <dt>Função</dt>
                  <dd>{classDraft.specialization}</dd>
                </div>
                <div>
                  <dt>Atributo</dt>
                  <dd>{classDraft.primaryAttribute}</dd>
                </div>
              </>
            ) : null}
            {kind === "race" ? (
              <>
                <div>
                  <dt>HP</dt>
                  <dd>{raceDraft.baseHp}</dd>
                </div>
                <div>
                  <dt>Bônus</dt>
                  <dd>{Object.values(raceDraft.bonuses).reduce((a, b) => a + b, 0)}/25</dd>
                </div>
              </>
            ) : null}
            {kind === "item" ? (
      ...
      <div className={styles.contentSave}>
        <div>
          ...
          <small>Classes e raças novas entram como rascunho para revisão.</small>
        </div>
...
function ClassForm({
  ...
  value: SimpleClassDraft;
  onChange: (v: SimpleClassDraft) => void;
}) {
  return (
    <>
      <Text label="Nome" value={value.name} onChange={(name) => onChange({ ...value, name })} />
      <TextArea
        label="Descrição para jogadores"
      ...
      <div className={styles.grid}>
        <Text
          ...
          onChange={(specialization) => onChange({ ...value, specialization })}
        />
        <Select
          label="Atributo principal"
          value={value.primaryAttribute}
          options={attributes.map((a) => [a, a])}
          onChange={(v) =>
            onChange({ ...value, primaryAttribute: v as SimpleClassDraft["primaryAttribute"] })
          }
        />
        <NumberField
        ...
        <Text
          ...
          onChange={(resourceName) => onChange({ ...value, resourceName })}
        />
        <NumberField
          label="Máximo do recurso"
          value={value.resourceMaximum}
          min={1}
          max={999}
          onChange={(resourceMaximum) => onChange({ ...value, resourceMaximum })}
        />
        <Text
          label="Imagem (URL)"
        ...
        <Text
          ...
          value={value.passiveName}
          onChange={(passiveName) => onChange({ ...value, passiveName })}
        />
      </div>
      <TextArea
        label="Descrição da passiva"
        value={value.passiveDescription}
        onChange={(passiveDescription) => onChange({ ...value, passiveDescription })}
      />
    </>
  );
}
function RaceForm({
  ...
  value: SimpleRaceDraft;
  onChange: (v: SimpleRaceDraft) => void;
}) {
  return (
    <>
      <Text label="Nome" value={value.name} onChange={(name) => onChange({ ...value, name })} />
      ...
      <div className={styles.grid}>
        ...
        <NumberField
          ...
          min={150}
          max={800}
          onChange={(baseHp) => onChange({ ...value, baseHp })}
        />
        <NumberField
          label="Mana inicial"
          value={value.baseMana}
          min={0}
          max={999}
          onChange={(baseMana) => onChange({ ...value, baseMana })}
        />
        <Text
          label="Imagem (URL)"
          value={value.imageUrl}
        ...
        <Text
          ...
          onChange={(traitName) => onChange({ ...value, traitName })}
        />
      </div>
      <TextArea
        label="Descrição do traço racial"
        value={value.traitDescription}
        onChange={(traitDescription) => onChange({ ...value, traitDescription })}
      />
      <Attributes
        label="Bônus raciais · máximo 25"
        value={value.bonuses}
...
function Attributes({
  ...
  onChange: (v: AttributeDraft) => void;
}) {
  return (
