...
export function WorldMap({ linkToRealms = true }: { linkToRealms?: boolean }) {
  return <section className="world-map world-map--illustrated" aria-label="Mapa interativo dos seis reinos de Wonderland">
    ...
    <div className="world-map__territories">
      {realmLore.map((realm,index)=>{const style={"--realm":realm.color,"--territory":territoryShapes[realm.key],"--pin-x":`${realm.position[0]}%`,"--pin-y":`${realm.position[1]}%`} as React.CSSProperties;const pin=<span className="world-map__pin"><b>{realm.icon}</b><i>{String(index+1).padStart(2,"0")}</i></span>;return linkToRealms?<Link className={`world-map__territory is-${realm.key}`} href={`/reinos#${realm.key}`} key={realm.key} style={style} aria-label={`Explorar ${realm.name}`}>{pin}</Link>:<a className={`world-map__territory is-${realm.key}`} href={`#${realm.key}`} key={realm.key} style={style} aria-label={`Ver imagens e informações de ${realm.name}`}>{pin}</a>;})}
