...
import { parseItemPayload, sumItemBonuses, type ItemPayload } from "@/lib/game/items";
...
async function loadSheets(rows: CharacterRow[]): Promise<CharacterSheet[]> {
  ...
  const { data: inventoryData, error: inventoryError } = await client
    ...
    .select("id, character_id, item_id, equipped_slot")
    .in("character_id", characterIds)
    .not("equipped_slot", "is", null);
  const inventoryRows = inventoryError?.code === "42P01" ? [] : (inventoryData ?? []);
  const ids = [
    ...new Set([
      ...records.flatMap((entry) => [entry.race_id, entry.class_id]),
      ...inventoryRows.map((entry) => entry.item_id),
    ]),
  ];
  ...
  const [characterRules, combatRules] = await Promise.all([getCharacterRules(), getCombatRules()]);
  return records.flatMap((record) => {
    ...
    const equippedItems = inventoryRows.flatMap((inventory) => {
      if (inventory.character_id !== record.id || !inventory.equipped_slot) return [];
      const itemRow = content.get(inventory.item_id);
      if (!itemRow) return [];
      const item = parseItemPayload(itemRow.payload);
      ...
      return [
        {
          ...
          itemId: itemRow.id,
          name: itemRow.name,
          slot: inventory.equipped_slot,
          payload: item.data,
        },
      ];
    });
    const equipmentBonuses = sumItemBonuses(equippedItems);
    return [
      {
        ...record,
