...
const formatSchema = z.enum(["solo", "duo", "trio"]);
...
export type QueueState = {
  ...
  opponentTertiary?: QueueCharacter | null;
...
function validCharacters(value: unknown): value is QueueCharacter[] {
  return (
    Array.isArray(value) &&
    value.every(
      (entry) => isObject(entry) && typeof entry.id === "string" && typeof entry.name === "string",
    )
  );
}
...
export async function joinPvpQueueAction(
  characterId: string,
  format: "solo" | "duo" | "trio" = "solo",
) {
...
export async function respondPvpMatchAction(matchId: string, accept: boolean) {
  ...
  if (error || !validResult(data))
    return {
      ok: false as const,
      message: error?.message ?? "Não foi possível confirmar a partida.",
    };
  return { ok: true as const, data };
}
...
export async function getPvpPartyStateAction(characterId: string) {
  ...
  const context = await rpcClient();
  if (!context) return { ok: false as const, message: "Arena indisponível." };
  const { data, error } = await context.rpc("v2_get_pvp_party_state", {
    p_character_id: parsed.data,
  });
  if (error || !validPartyState(data))
    return {
      ok: false as const,
      message: error?.message ?? "Não foi possível consultar sua equipe.",
    };
  return { ok: true as const, data };
}
...
export async function searchPvpPartnerAction(characterId: string, query: string) {
  ...
  if (error || !validCharacters(data))
    return {
      ok: false as const,
      message: error?.message ?? "Não foi possível procurar jogadores.",
    };
  return { ok: true as const, data };
}
...
export async function cancelPvpPartyInviteAction(inviteId: string) {
  ...
  const { error } = await client.rpc(
    "v2_cancel_pvp_party_invite" as never,
    { p_invite_id: parsed.data } as never,
  );
  return { ok: !error } as const;
}
