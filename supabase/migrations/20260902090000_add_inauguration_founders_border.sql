-- Libera a moldura comemorativa para a prévia administrativa.
-- A distribuição aos participantes continuará fechada até a lista oficial ser confirmada.
...
begin
  ...
  if p_key is not null and btrim(p_key) <> '' and not (
    ...
    (p_slot='border' and p_key in ('trono-rei-oco','moldura-fundadores-2026'))
  ...
  update public.v2_characters set cosmetics=next_loadout,updated_at=now() where id=p_character_id and user_id=caller_id;
  insert into public.v2_admin_history(actor_id,action,target_type,target_id,details)
  values(caller_id,case when p_key is null or btrim(p_key)='' then 'character.cosmetic.removed' else 'character.cosmetic.equipped' end,'character_cosmetic',p_character_id,jsonb_build_object('slot',p_slot,'key',p_key,'collection',case when p_key='moldura-fundadores-2026' then 'inauguracao-2026' else 'halloween-2026' end));
  return next_loadout;
end;
$$;
