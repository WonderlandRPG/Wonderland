...
import { getCharacterSheet, getCharacterSheets, type CharacterSheet } from "@/lib/content/characters";
...
export default async function ArenaPage({
  ...
  const { data: pvpQueue } = pvpClient && query.partida
    ? await pvpClient.from("v2_pvp_queue").select("opponent_character_id,match_id,status").eq("match_id", query.partida).eq("user_id", account.id).eq("status", "matched").maybeSingle()
    : { data: null };
  const opponent = pvpQueue?.opponent_character_id ? await getCharacterSheet(pvpQueue.opponent_character_id) : null;
  const toArenaCharacter = (character: CharacterSheet) => ({
    ...
    usesMana: [...character.unlockedClassSkills, ...character.unlockedRaceAbilities].some((skill) => skill.resource === "mana"),
    attributes: character.stats.attributes,
    skills: character.unlockedClassSkills.filter((skill) => !/passiva/i.test(skill.type)).map(prepareArenaSkill),
    raceAbilities: character.unlockedRaceAbilities,
    combatLore: [
      { name: character.characterClass.payload.passive.name, description: character.characterClass.payload.passive.description },
      { name: character.characterClass.payload.mechanic.name, description: character.characterClass.payload.mechanic.description },
      ...character.characterClass.payload.paths.filter((path) => path.key === character.class_path_key).map((path) => ({ name: path.passive.name, description: path.passive.description })),
      ...character.race.payload.traits,
      ...character.race.payload.mechanics,
      ...character.inventory.filter((item) => item.equippedSlot).flatMap((item) => item.specialEffects.map((effect) => ({ name: effect.name, description: effect.description }))),
    ],
    equipmentEffects: character.inventory.filter((item) => item.equippedSlot).flatMap((item) => item.specialEffects),
    items: character.inventory.filter((item) => /consum|poção|pocao/i.test(item.category)).map((item) => ({ id: item.id, name: item.name, description: item.description })),
  });
  ...
  return (
    <main className="arena-page">
      ...
      <div className="page-container arena-page__inner">
        ...
        {mode === "pvp" && activeCharacter && !opponent ? <PvpLobby characterId={activeCharacter.id} characterName={activeCharacter.name} rank={activeCharacter.adventure_rank} /> : null}
        {mode && (mode !== "pvp" || opponent) ? <>
        <Link className="arena-mode-back" href="/arena">← Trocar modo</Link>
        <TrainingArena
          characters={characters.filter((character) => character.id === characterId).map(toArenaCharacter)}
