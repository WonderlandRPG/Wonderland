...
-- Remove boilerplate repeated on every generated contract and a duplicated
-- "antes" introduced by the old template combinator.
update public.v2_missions
set description = trim(regexp_replace(
  regexp_replace(description, '[[:space:]]+A ocorrência ainda é localizada e permite conversar, observar e testar uma solução antes que alguém se machuque\.$', ''),
  '(antes [^.]+) antes da chegada da ameaça', '\1', 'i'
))
...
begin
  ...
  with eligible as (
    ...
    select * from weekly_selection
    union all
    select m.* from public.v2_missions m
    where m.active and m.kingdom=chosen.kingdom and m.rank=chosen.adventure_rank
