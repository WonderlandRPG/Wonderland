...
function makePlayer(character: TacticalCharacter) {
  return createCombatant({
    ...
    // As antigas raças possuem BASIC_ATTACK_HIT genérico. No tático, a geração
    // racial é regida pelas Reações oficiais, evitando +1 incorreto/duplicado.
...
export function TacticalLabV6({
  ...
  const [message, setMessage] = useState("V6: execução completa de habilidades carregada.");
  const [log, setLog] = useState<string[]>([
    "V6: skills, reações, alcance, controles e movimento usando regras táticas centralizadas.",
  ]);
  ...
  function addLog(text: string) {
    setLog((current) => [text, ...current].slice(0, 18));
  }
  ...
  function resetBoard(nextCharacter = character, nextCreature = creature) {
    ...
    resetTurnActions();
    setRound(1);
    setLog([`V6 reiniciada: ${nextCharacter.name} vs ${nextCreature.name}.`]);
  ...
  function selectAction(next: PlayerAction) {
    ...
    if (next.kind === "basic") {
      ...
      if (sourceUsed(next.source)) {
        return setMessage(`Habilidade de ${next.source === "class" ? "Classe" : "Raça"} já usada neste turno.`);
      }
  ...
  function executePlayerSkill(selected: Extract<PlayerAction, { kind: "skill" }>, center: TacticalPosition) {
    ...
    const controlText = controlResult.resisted.length
      ...
      : "";
    const text = `${result.event.message}${traitText}${controlText}${spatial.messages.length ? ` ${spatial.messages.join(" ")}` : ""}`;
    setMessage(text);
    addLog(text);
    clearAction();
  }
  ...
  function clickCell(position: TacticalPosition) {
    ...
    if (action.kind === "basic") {
      ...
      if (!hasTacticalLineOfSight({ from: playerPosition, to: enemyPosition, blocked: obstacles })) {
      ...
      const resistanceText = resistanceResult.resistance
        ...
        : "";
      const text = `${result.event.message}${resistanceText}${reactions.messages.length ? ` ${reactions.messages.join(" ")}` : ""}`;
      setPlayerState(reactions.actor);
    ...
    if (targetsEnemy && !hasTacticalLineOfSight({ from: playerPosition, to: position, blocked: obstacles })) {
  ...
  function executeEnemyTurn() {
    ...
    if (currentEnemyStun > 0) {
      ...
      const hasReadySkill =
        currentEnemySilence <= 0 && creatureSkills.some((skill) => (nextEnemy.cooldowns[skill.key] ?? 0) <= 0);
      ...
      if (selectedEnemySkill) {
        ...
        const received = Math.max(0, beforePlayer.hp + beforePlayer.shield - (nextPlayer.hp + nextPlayer.shield));
        if (received > 0) notes.push(`${player.name} recebeu ${received} de dano nesta ação.`);
        ...
        notes.push(`${result.event.message}${reactions.messages.length ? ` ${reactions.messages.join(" ")}` : ""}`);
      } else {
  ...
  return (
    <section className={styles.lab} aria-label="Laboratório do mapa tático V6">
      <header className={styles.header}>
        <div>
          <span className={styles.eyebrow}>Somente ADM · motor tático V6</span>
          <h1>Laboratório do Mapa Tático</h1>
          <p>Execução funcional de skills, recursos, reações, alcance, controles e operações espaciais.</p>
        </div>
        <div className={styles.status}><small>Funcional</small><strong>V6 · Rodada {round}</strong></div>
      </header>
      ...
      <section className={styles.characterPanel} data-wl-surface="raised">
        <label>
          ...
          <select value={character.id} onChange={(event) => changeCharacter(event.target.value)}>
            {characters.map((entry) => <option key={entry.id} value={entry.id}>{entry.name} · Rank {entry.rank} · {entry.className}</option>)}
          </select>
        </label>
        <label>
          <span>Criatura do Bestiário</span>
          <select value={creature.id} onChange={(event) => changeCreature(event.target.value)}>
            {creatures.map((entry) => <option key={entry.id} value={entry.id}>{entry.name} · Rank {entry.rank} · {entry.combatProfile.aiProfile}</option>)}
          </select>
        </label>
        <div className={styles.characterSummary}>
          <strong>{character.name}</strong>
          <span>{character.raceName} · {character.className} · Rank {character.rank}</span>
          <span>{rankMismatch ? `TESTE FORA DO RANK: criatura Rank ${creature.rank}` : `Pareamento de Rank válido: ${creature.rank}`}</span>
        </div>
      </section>

      <section className={styles.combatHud}>
        <article>
          <small>AVENTUREIRO</small><strong>{player.name}</strong>
          ...
          {playerRoot > 0 ? <span>ROOT: {playerRoot}</span> : null}{playerStun > 0 ? <span>STUN: {playerStun}</span> : null}
          {playerSilence > 0 ? <span>SILENCE: {playerSilence}</span> : null}{playerFear > 0 ? <span>FEAR: {playerFear}</span> : null}
          {playerTaunt ? <span>TAUNT: {playerTaunt.turns}</span> : null}
        </article>
        <article data-enemy="true">
          <small>BESTIÁRIO · RANK {creature.rank}</small><strong>{creature.name}</strong>
          ...
          <span>Fraquezas: {creature.weaknesses.length ? creature.weaknesses.join(" · ") : "nenhuma"}</span>
          <span>Resistências: {profile.resistances.length ? profile.resistances.join(" · ") : "nenhuma"}</span>
          {enemyRoot > 0 ? <span>ROOT: {enemyRoot}</span> : null}{enemyStun > 0 ? <span>STUN: {enemyStun}</span> : null}
          {enemySilence > 0 ? <span>SILENCE: {enemySilence}</span> : null}{enemyFear > 0 ? <span>FEAR: {enemyFear}</span> : null}
      ...
      <div className={styles.skillBar} data-wl-surface="raised">
        {character.skills.map(({ source, skill }) => {
          ...
          const blockedByFear = playerFear > 0 && affectsEnemy(skill);
          const blocked = sourceUsed(source) || cooldown > 0 || playerSkillBlocked || blockedByFear || finished;
          return (
            <button key={`${source}-${skill.key}`} type="button" disabled={blocked} data-selected={action?.kind === "skill" && action.skill.key === skill.key ? "true" : "false"} onClick={() => selectAction({ kind: "skill", name: skill.name, range: skill.range, area: skill.area, source, skill })}>
        ...
        {character.items.map((item) => (
          <button key={item.id} type="button" disabled={usedItem || playerStun > 0 || finished} onClick={() => useItem(item)}>
            <strong>{item.name}</strong><span>Item ativo · 1 por turno</span><small>{usedItem ? "USADO" : "DISPONÍVEL"}</small>
          </button>
        ))}
      </div>
            ...
            {cells.map((position) => {
              ...
              return (
                <button key={key} type="button" className={styles.cell} data-state={state} onClick={() => clickCell(position)} aria-label={isPlayer ? player.name : isEnemy ? creature.name : isObstacle ? "Obstáculo" : `Casa ${position.x + 1}, ${position.y + 1}`}>
                  {isPlayer ? <span className={styles.unit}>♞</span> : null}{isEnemy ? <span className={styles.unit}>♜</span> : null}{isObstacle ? <span className={styles.obstacle}>◆</span> : null}
                </button>
              );
            })}
          </div>
