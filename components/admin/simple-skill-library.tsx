...
export function SimpleSkillLibrary({ classes }: { classes: ClassRow[] }) {
  ...
  const selected =
    currentClass?.skills.find((item) => item.key === skillKey) ?? currentClass?.skills[0];
  ...
  const patch = <K extends keyof SimpleSkillDraft>(key: K, value: SimpleSkillDraft[K]) =>
    setDraft((current) => (current ? { ...current, [key]: value } : current));
  function save() {
    if (!draft || !currentClass || !skillKey) return;
    startTransition(async () => {
      const result = await replaceSkillInClassAction({
        classId: currentClass.id,
        existingKey: skillKey,
        draft,
      });
  ...
  return (
    <section className={styles.shortcuts} style={{ marginTop: 24 }}>
      <div className={styles.sectionHeading}>
        <div>
          <span>05</span>
          <h2>Editar habilidade existente · modo simples</h2>
        </div>
      ...
      <div className={styles.grid}>
        <label>
          <span>Classe</span>
          <select value={classId} onChange={(e) => chooseClass(e.target.value)}>
            {classes.map((item) => (
        ...
        <label>
          ...
          <select value={skillKey} onChange={(e) => chooseSkill(e.target.value)}>
            {(currentClass?.skills ?? []).map((item) => (
              <option key={item.key} value={item.key}>
                Nv. {item.level} · {item.name}
              </option>
            ))}
          </select>
        </label>
      ...
      {!draft ? (
        <p>Nenhuma habilidade universal cadastrada nesta classe.</p>
      ) : (
        <div className={styles.formCard} style={{ marginTop: 16 }}>
          <label className={styles.full}>
            <span>Texto para o jogador</span>
            <textarea
              rows={4}
          ...
          <div className={styles.grid}>
            ...
            <label>
              ...
              <select
                ...
                onChange={(e) =>
                  patch("attribute", e.target.value as SimpleSkillDraft["attribute"])
                }
              >
                {["FOR", "DEF", "RES", "INI", "INT"].map((value) => (
            ...
            <label>
              ...
              <input
                ...
                onChange={(e) => patch("baseValue", Number(e.target.value))}
              />
            </label>
            <label>
              <span>Custo</span>
          ...
          <button className={styles.aiButton} disabled={pending} onClick={save} type="button">
            {pending ? "Salvando…" : "Salvar edição simples"}
          </button>
