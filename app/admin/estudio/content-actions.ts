...
function specialEffects(input: { id?:string; name:string; effectKind:string; effectName:string; effectDescription:string; effectPower:number; effectDuration:number }) {
  ...
  return [{ key:`${input.id || slugify(input.name)}-studio-effect`, kind:input.effectKind, name:input.effectName.replace(/\bARC\b/g,"HP"), description:(input.effectDescription || "Efeito configurado pelo Studio de Criação.").replace(/\bARC\b/g,"HP"), trigger:input.effectKind === "COOLDOWN_REDUCTION" ? "ON_SKILL_USE" : "ON_DAMAGE_DEALT", duration:input.effectDuration, power:input.effectPower, modifiers:{} }];
}
...
export async function saveSimpleItemAction(input: unknown) {
  ...
  const payload={name:d.name.replace(/\bARC\b/g,"HP"),description:d.description.replace(/\bARC\b/g,"HP"),category:d.category,slot:d.slot,rarity:d.rarity,price:d.price,image_url:d.imageUrl||null,attributes:attributesWithoutZero(d.attributes),special_effects:specialEffects(d),two_handed:d.twoHanded,active:true,updated_at:new Date().toISOString()}; let id=d.id;
...
export async function saveSimpleTitleAction(input: unknown) {
  ...
  const payload={name:d.name.replace(/\bARC\b/g,"HP"),description:d.description.replace(/\bARC\b/g,"HP"),category:"Título",price:0,slot:"title",rarity:"awakened",attributes:d.attributes,title_style:{primary:d.primary,secondary:d.secondary,glow:d.glow},special_effects:specialEffects(d),two_handed:false,active:false,updated_at:new Date().toISOString()}; let id=d.id;
  ...
  await history(account.id,d.id?"title.updated_from_studio":"title.created_from_studio","title",id,{name:d.name}); revalidatePath("/admin/titulos"); revalidatePath("/admin/console"); revalidatePath("/admin/eventos"); revalidatePath("/arena"); revalidatePath("/admin/estudio"); return {ok:true as const,message:`${d.name} foi ${d.id?"atualizado":"criado"}.`,id};
}
...
function schemaFor(kind:StudioContentKind){
  ...
  const itemAttrs={type:"object",additionalProperties:false,required:["FOR","DEF","RES","INI","INT","HP"],properties:Object.fromEntries(["FOR","DEF","RES","INI","INT","HP"].map(k=>[k,{type:"integer",minimum:0,maximum:999}]))};
