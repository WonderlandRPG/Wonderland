...
(function(){
  ...
  const state={module:"races",rows:[],selected:null,classes:[],races:[],paths:[],source:"db"};
  ...
  function field(label,name,value,type="text",wide=false){
    return `<label class="admin-field ${wide?"admin-field-wide":""}"><span>${esc(label)}</span><input name="${name}" type="${type}" value="${esc(value??"")}"></label>`;
  }
  ...
  function render(){
    ...
    host.innerHTML=`<section class="admin-cms-hero"><div><span>Biblioteca viva</span><h3>${labels[state.module]}</h3><p>Edite o conteúdo do RPG sem tocar no código. Registros locais podem ser importados para o banco.</p></div><div class="admin-cms-actions"><button id="cmsImport" class="wl-button wl-button-ghost" type="button">Importar conteúdo atual</button><button id="cmsCreate" class="wl-button wl-button-gold" type="button">Criar novo</button></div></section><section class="admin-cms-toolbar"><input id="cmsSearch" type="search" placeholder="Pesquisar por nome, classe, raça ou chave..."><span>${state.rows.length} registro(s)</span></section><section class="admin-browser admin-cms-browser"><aside class="admin-record-list" id="cmsList">${state.rows.map(r=>`<button class="admin-record-button ${String(r[key])===String(state.selected)?"active":""}" data-id="${esc(r[key])}"><small>${esc(r.source_type||r.class_id||r.id||r[key])}${r._local?" • local":""}</small><strong>${esc(r.name||r.skill_key||r.passive_key||r.mechanic_key)}</strong></button>`).join("")||'<div class="admin-empty">Nenhum registro encontrado.</div>'}</aside><div class="admin-detail-pane">${editor(selected)}</div></section>`;
    bind();
  }
  ...
  function bind(){
    ...
    document.getElementById("cmsSearch")?.addEventListener("input",e=>{const q=e.target.value.toLowerCase();host.querySelectorAll("#cmsList [data-id]").forEach(b=>b.hidden=!b.textContent.toLowerCase().includes(q))});
    ...
    form.addEventListener("change",e=>{if(e.target.name==="source_type"){Object.assign(row,readForm(form,row));render()}});
    form.addEventListener("submit",async e=>{e.preventDefault();msg.textContent="Salvando...";try{const payload=readForm(form,row),table=tables[state.module],isNew=String(row[key]||"").startsWith("novo-")||row._local;let result;if(isNew)result=await client.from(table).insert(payload).select("*").single();else result=await client.from(table).update(payload).eq(key,row[key]).select("*").single();if(result.error)throw result.error;msg.textContent="Salvo com sucesso.";await load(state.module,result.data[key])}catch(err){console.error(err);msg.textContent=err.message||"Não foi possível salvar."}});
    document.getElementById("cmsDelete")?.addEventListener("click",async()=>{if(!confirm("Excluir este registro?"))return;if(row._local||String(row[key]).startsWith("novo-")){state.rows=state.rows.filter(x=>x!==row);state.selected=null;render();return}const {error}=await client.from(tables[state.module]).delete().eq(key,row[key]);if(error){msg.textContent=error.message;return}await load(state.module)});
  }

  async function load(module=state.module,selected=null){
    state.module=module;
    const content=store?await store.load():null;
    state.classes=content?.classes||[];state.races=content?.races||[];state.paths=content?.paths||[];
    const key=keys[module],table=tables[module];
    let {data,error}=await client.from(table).select("*").order(key);
    if(error){host.innerHTML=`<div class="admin-error">${esc(error.message)}</div>`;return}
    const fallback=content?content[module]||[]:[];
    const db=data||[],used=new Set(db.map(r=>String(r[key])));
    state.rows=[...db,...fallback.filter(r=>!used.has(String(r[key]))).map(r=>({...r,_local:true}))];
    state.selected=selected||state.rows[0]?.[key]||null;
    render();
  }

  [["races","races"],["classes","classes"],["skills","skills"]].forEach(([attr,key])=>document.querySelector(`[data-admin-module="${attr}"]`)?.addEventListener("click",e=>{e.preventDefault();e.stopImmediatePropagation();load(key)},true));
