...
import { LoreCollapse } from "@/components/lore/lore-collapse";
      ...
      <div className={styles.shell}>
        <header className={styles.hero}>
          <div>
            ...
            <p>
              Aqui ficam preservados o nascimento do mundo, a origem da magia e os acontecimentos
              que formam o cânone oficial do RPG. Abaixo das eras, a Biblioteca Real recebe novos
              contos e histórias publicados pelos escribas da administração.
            </p>
          </div>
        ...
        <section className={styles.library} id="biblioteca">
          <header className={styles.libraryHeader}>
            <div>
              <small>NOVAS OBRAS · ACERVO VIVO</small>
              <h2>Contos da Biblioteca</h2>
            </div>
            <p>
              Histórias posteriores ao registro original ficam guardadas como livros. Abra uma capa
          ...
          <div className={styles.bookGrid}>
            {stories.length ? (
                    ...
                    >
                      <div>
                        <span className={styles.bookNumber}>
                          Tomo {String(stories.length - index).padStart(2, "0")}
                        </span>
                        <h3>{story.name}</h3>
                        <p>{payload.excerpt}</p>
                      </div>
                      <footer>
                        ...
                        <small>
                          {new Date(`${payload.publishedOn}T12:00:00Z`).toLocaleDateString(
                            "pt-BR",
                            { timeZone: "UTC" },
                          )}
                        </small>
                    ...
                    {canWrite ? (
                      ...
                      >
                        ...
                        <button
                          ...
                          style={{ minHeight: 34, padding: "7px 11px" }}
                        >
                          Apagar história
                        </button>
                      </form>
              ...
              <div className={styles.emptyShelf}>
                <strong>A estante de novos contos ainda está vazia.</strong>
                <p>Quando um ADM publicar uma nova história, ela aparecerá aqui como um livro.</p>
              </div>
            )}
          </div>
        </section>
        ...
        {canWrite ? (
          <details className={styles.adminWrite}>
            <summary>
              <span>✒ Mesa do escriba</span>
              <b>Publicar um conto sem sair da História</b>
            </summary>
            <div className={styles.adminWriteBody}>
              <form action={saveLoreStoryAction} className={styles.adminForm}>
                ...
                <div className={styles.adminFormGrid}>
                  ...
                  <label>
                    <span>Autor</span>
                    <input name="authorName" defaultValue={account?.displayName} />
                  </label>
                  <label>
                    <span>Data</span>
                    <input
                ...
                <div className={styles.adminFormGrid}>
                  <label>
                    <span>Resumo</span>
                    <textarea name="excerpt" maxLength={320} />
                  </label>
                  <label>
                    <span>Capa</span>
                    <select name="coverTone" defaultValue="forest">
                      <option value="forest">Verde da Guilda</option>
                      <option value="wine">Vinho Real</option>
                      <option value="midnight">Azul da Meia-noite</option>
                      <option value="royal">Dourado Real</option>
                      <option value="ember">Brasa Carmesim</option>
                      <option value="ocean">Azul Oceânico</option>
                    </select>
                  </label>
                  <label>
                ...
                <label>
                  <input name="published" type="checkbox" defaultChecked /> Publicar imediatamente
                </label>
