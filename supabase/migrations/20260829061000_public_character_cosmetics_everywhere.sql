...
as $$
  select
    c.id,c.user_id,c.name,c.level,c.xp,r.name,cl.name,c.image_url,c.kingdom,c.adventure_rank,
...
as $$
  select
    c.id,c.user_id,c.name,c.level,c.image_url,r.name,cl.name,c.adventure_rank,
    count(h.id),
    count(h.id) filter(where h.result='victory'),
    count(h.id) filter(where h.result='defeat'),
...
begin
  ...
  return jsonb_build_object(
    'character',jsonb_build_object(
      ...
      'cosmetics',coalesce(opponent.cosmetics,'{}'::jsonb)
    ),
    'equipment',equipment
  );
end;
...
begin
  ...
  v_users:=array_remove(array[
    room.player_one_user_id,
    room.player_one_secondary_user_id,
    room.player_two_user_id,
    room.player_two_secondary_user_id
  ],null);
    ...
    jsonb_agg(
      jsonb_build_object(
        ...
        'character',jsonb_build_object(
          ...
          'race_id',c.race_id,
          'class_id',c.class_id,
          'class_path_key',c.class_path_key,
          'level',c.level,
          'image_url',c.image_url,
          'adventure_rank',c.adventure_rank,
          'allocated_attributes',c.allocated_attributes,
          'cosmetics',coalesce(c.cosmetics,'{}'::jsonb)
        ),
        'equipment',coalesce((
          select jsonb_agg(
            jsonb_build_object(
          ...
          from public.v2_character_inventory i
          where i.character_id=c.id
        ),'[]'::jsonb)
      )
      order by x.team,x.slot
    ),
    '[]'::jsonb
  ) into result
  from(values
  ...
  return jsonb_build_object(
    'format',room.format,
    'ownTeam',case
      when v_user=any(array_remove(array[room.player_one_user_id,room.player_one_secondary_user_id],null))
      then 1 else 2
    end,
    'members',result
  );
