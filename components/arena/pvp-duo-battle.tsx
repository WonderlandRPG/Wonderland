...
export function PvpDuoBattle({
  ...
  const isMyTurn = state.status === "active" && ownIds.includes(activeId);
...
function DuoFighter({ fighter, character, active, fx }: { fighter?: CombatantState; character?: ArenaCharacter; active: boolean; fx: Fx }) {
  if (!fighter || !character) return null;
  return (
    <article className={`pvp-fighter jrpg-fighter duo-fighter ${active ? "is-active" : ""} ${fighter.hp <= 0 ? "is-down" : ""} ${fx ? `fx-${fx.kind}` : ""}`}>
