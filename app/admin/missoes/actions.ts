...
export async function createMissionAction(formData:FormData){
  ...
  const {error}=await client.from("v2_missions").insert({slug:parsed.data.slug,name:parsed.data.name,description:parsed.data.description,objective:parsed.data.objective,kingdom:parsed.data.kingdom,rank:parsed.data.rank,min_level:parsed.data.minLevel,reward_xp:parsed.data.rewardXp,reward_gold:parsed.data.rewardGold,is_rank_trial:parsed.data.isRankTrial,promotion_rank:parsed.data.promotionRank,created_by:account.id});
