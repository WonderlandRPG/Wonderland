...
export function parseMissionBoard(value: Json | null): MissionBoard | null {
  const root = record(value),
    character = record(root.character);
  if (!text(character.id)) return null;
  const missions = Array.isArray(root.missions)
    ? root.missions
        .map((entry) => {
  ...
  return {
    character: {
      ...
      kingdom: text(character.kingdom),
      imageUrl: text(character.imageUrl) || null,
    },
    missions,
    activeAssignment: text(rawAssignment.id)
      ? {
          id: text(rawAssignment.id),
          missionId: text(rawAssignment.missionId),
          name: text(rawAssignment.name),
          description: text(rawAssignment.description),
          rank: text(rawAssignment.rank),
          kingdom: text(rawAssignment.kingdom),
    ...
    ? value
        .map((entry) => {
          ...
          return {
            ...
            characterRank: text(row.characterRank),
            characterLevel: number(row.characterLevel),
            missionName: text(row.missionName),
            missionDescription: text(row.missionDescription),
            missionObjective: text(row.missionObjective),
            missionRank: text(row.missionRank),
            kingdom: text(row.kingdom),
