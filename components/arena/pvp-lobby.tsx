...
export function PvpLobby({
  ...
  const [format, setFormat] = useState<"solo" | "duo">(ranked ? "duo" : "solo");
  ...
  useEffect(() => {
    if (queue?.status === "matched" && queue.matchId && queue.acceptanceStatus === "ready") {
      if (queue.format === "duo") router.replace(`/combate/pvp-duo/${queue.matchId}`);
      else router.replace(`/combate?modo=pvp&partida=${queue.matchId}`);
    }
  }, [queue, router]);
  ...
  function respond(inviteId: string, accept: boolean) {
    ...
    startTransition(async () => {
      ...
      if (!result.ok) setMessage(result.message);
        ...
        setMessage(
          accept
            ? "Dupla formada! Vocês permanecerão juntos até alguém desfazer a dupla."
            : "Convite recusado.",
        );
        await refreshParty();
      }
  ...
  return (
    <section className={`pvp-lobby ${matched ? "is-matched" : ""}`}>
      ...
      <span className="eyebrow">{ranked ? "Ranqueada 2 × 2" : "Combate competitivo"}</span>
      ...
      <p>
        {matched
          ...
          : ranked
            ? "Forme sua dupla. A fila ranqueada prioriza adversários da mesma faixa competitiva."
            ...
            <>
              ...
              {!partyUsesActiveCharacter ? (
                <p className="pvp-party-warning">
                  Esta dupla foi formada com <strong>{party.ownCharacter.name}</strong>. Selecione
                  esse personagem como ativo para entrar no 2x2.
                </p>
              ) : null}
      ...
      {matched && queue.matchId && queue.acceptanceStatus !== "ready" ? (
        ...
        >
          <section className="pvp-ready-check">
            ...
            <small>COMBATE PvP · {activeFormat === "duo" ? "2 × 2" : "1 × 1"}</small>
            ...
            <b>
              ...
              {queue.requiredCount ?? (activeFormat === "duo" ? 4 : 2)} confirmaram
            </b>
            {queue.acceptedByYou ? (
      ...
      {queue?.status === "searching" ? (
        <>
          ...
          <em>
            ...
            {activeFormat === "duo" ? "outra dupla" : "outro personagem"} Rank {rank}…
          </em>
        </>
      ) : null}
