...
(function(){
  ...
  async function getCharacters(){
    ...
    const fields="id,name,story,race_id,class_id,path_id,kingdom_id,guild_rank,level,experience,wg,hp_current,mana_current,distribution_profile,image_url,created_at";
  ...
  async function getCharacterSheet(characterId){
    ...
    const fields="id,user_id,name,story,race_id,class_id,path_id,kingdom_id,guild_rank,level,experience,wg,hp_current,mana_current,distribution_profile,image_url,created_at";
  ...
  async function createCharacter(character){
    const user=await current();if(!user)throw new Error("Sessão expirada. Entre novamente.");
    const payload={user_id:user.id,name:character.name,story:character.story||"",race_id:character.raceId,class_id:character.classId,path_id:character.pathId||null,kingdom_id:character.kingdomId||null,guild_rank:"E",level:1,experience:0,wg:1500,hp_current:Number(character.hpCurrent||0),mana_current:Number(character.manaCurrent||0),distribution_profile:character.attributeProfile==="custom"?"manual":character.attributeProfile||"manual",image_url:character.image||null};
