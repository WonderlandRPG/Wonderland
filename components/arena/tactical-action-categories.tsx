...
export function TacticalActionCategories({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    ...
    const lab = document.querySelector<HTMLElement>('[aria-label="Combate tático"]');
    if (!lab) return;
    const children = Array.from(lab.children) as HTMLElement[];
    const nextToolbar =
      children.find((entry) => entry.querySelector("button")?.textContent?.includes("Movimento")) ??
      null;
    const skillBar =
      children.find((entry) =>
        Array.from(entry.querySelectorAll("button")).some((button) =>
          /Classe · Alcance|Raça · Alcance|Item ativo/.test(button.textContent ?? ""),
        ),
      ) ?? null;
    ...
    const sync = () => {
      ...
      const group = (name: "race" | "class" | "item") =>
        buttons.filter((button) => button.dataset.actionGroup === name);
      setAvailability({
