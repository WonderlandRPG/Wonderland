...
export function PvpLobby({
  ...
  return (
    <section className={`pvp-lobby ${matched ? "is-matched" : ""}`}>
      ...
      <p>
        {matched
          ? "Sala criada. Aguarde enquanto os dois combatentes entram no campo."
          : "Encontre automaticamente outro aventureiro do mesmo Rank. Quando houver um adversário, a sala será aberta para os dois jogadores."}
      </p>
      <div>
        <article>
      ...
      {!queue || queue.status === "cancelled" || queue.status === "expired" ? (
        <button className="button button--primary" disabled={pending} onClick={join} type="button">
          {pending ? "Entrando…" : `Buscar oponente Rank ${rank}`}
        </button>
      ) : null}
      {queue?.status === "searching" ? (
        <>
          ...
          <em>
            <span className="signal-dot" /> Procurando outro personagem Rank {rank}…
          </em>
        </>
      ) : null}
