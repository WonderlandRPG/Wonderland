...
export default async function CharactersPage({ searchParams }: { searchParams: Promise<{ notice?: string; selecionar?: string; next?: string }> }) {
  ...
  return (
    <main className={`${styles.page} page`} data-character-hub>
      <PlayerNav />
      <div className={`${styles.inner} inner`}>
        <header className={`${styles.intro} intro`}>
          <div>
          ...
          {!selecting ? <div className={`${styles.online} online`}>Online em Wonderland</div> : characters.length < rules.maximumSlots ? <Link className="button button--primary" href="/personagens/novo">Criar personagem ＋</Link> : <span>{characters.length} / {rules.maximumSlots} fichas</span>}
        </header>

        {query.notice && noticeMessages[query.notice] ? <div className={styles.notice} role="status">{noticeMessages[query.notice]}</div> : null}

        {characters.length > 0 ? (
          <section className={`${styles.roster} roster ${!selecting ? `${styles.rosterLobby} rosterLobby` : ""}`}>
            {visibleCharacters.map((character) => {
              ...
              return (
                <article className={`${styles.heroCard} heroCard`} key={character.id} style={{ "--card-rank": rank.color } as React.CSSProperties}>
                  ...
                  <div className={`${styles.body} body`}>
                    ...
                    <dl className={`${styles.identity} identity`}><div><dt>Raça</dt><dd>{character.race.name}</dd></div><div><dt>Classe</dt><dd>{character.characterClass.name}</dd></div><div><dt>Rank</dt><dd>{rank.key}</dd></div></dl>
                    <p className={`${styles.wallet} wallet`}><span>Carteira</span><strong>{character.gold.toLocaleString("pt-BR")} WG</strong></p>
                    <div className={`${styles.stats} stats`}><span>HP <strong>{character.stats.maxHp}</strong></span><span>Recurso <strong>{character.characterClass.payload.resource.name}</strong></span><span>INI <strong>{character.stats.initiative}</strong></span></div>
                  </div>
            ...
            {!selecting && activeCharacter ? (
              <aside className={`${styles.session} session`}>
        ...
        {!selecting && activeCharacter ? (
          <section className={`${styles.commands} commands`}>
            <header><small>Escolha o próximo destino</small><h2>Para onde a jornada segue?</h2></header>
            <div className={`${styles.signs} signs`}>
              <Link className={`${styles.sign} sign`} href="/arena"><span>⚔</span><small>Combate</small><strong>Arena</strong></Link>
              <Link className={`${styles.sign} sign`} href={`/personagens/${activeCharacter.id}?tab=equipamentos`}><span>◈</span><small>Preparação</small><strong>Equipamentos</strong></Link>
              <Link className={`${styles.sign} sign`} href="/loja"><span>◆</span><small>Comércio</small><strong>Mercado</strong></Link>
              <div className={`${styles.sign} sign ${styles.disabled}`}><span>⌖</span><small>Exploração</small><strong>Mapa em manutenção</strong></div>
              <Link className={`${styles.sign} sign`} href="/eventos"><span>◇</span><small>Agenda</small><strong>Eventos</strong></Link>
              <Link className={`${styles.sign} sign`} href="/ranking"><span>♜</span><small>Prestígio</small><strong>Ranking</strong></Link>
            </div>
          </section>
        ) : null}

        {recentUpdates.length > 0 ? (
          <section className={`${styles.updates} updates`}>
            ...
            <div className={styles.updatesGrid}>
              {recentUpdates.map((update) => {
                ...
                return <Link className={`${styles.update} update`} href="/atualizacoes" key={update.id}><small>Versão {update.version}</small><h3>{update.title}</h3><p>{summary ?? "Abra as crônicas para conhecer os detalhes desta atualização."}</p></Link>;
              })}
            </div>
          </section>
        ) : null}
