...
import {
  ...
  type AttributeDraft, type ReworkItemAttributeDraft, type SimpleClassDraft, type SimpleItemDraft, type SimpleRaceDraft, type SimpleTitleDraft,
...
function ReworkItemAttributes({label,value,onChange}:{label:string;value:ReworkItemAttributeDraft;onChange:(v:ReworkItemAttributeDraft)=>void}){return <fieldset className={styles.attributeBox}><legend>{label}</legend><div className={styles.attributeGrid}>{officialItemAttributes.map((key)=><NumberField key={key} label={key} value={value[key]} min={0} max={999} onChange={(n)=>onChange({...value,[key]:n})}/>)}</div></fieldset>}
