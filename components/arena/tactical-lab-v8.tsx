...
function summonExpired(
  before: CombatantState["statuses"],
  after: CombatantState["statuses"],
) {
...
export function TacticalLabV8({
  ...
  const [message, setMessage] = useState("V8: skills, recursos e Doutrina do Caminho carregados.");
  const [log, setLog] = useState<string[]>([
    "V8: skills + recursos + reações + Doutrinas de Caminho em execução.",
  ]);
  ...
  function resetBoard(nextCharacter = character, nextCreature = creature) {
    ...
    resetTurnActions();
    setRound(1);
    setLog([`V8 reiniciada: ${nextCharacter.name} vs ${nextCreature.name}.`]);
  ...
  return (
    <section className={styles.lab} aria-label="Laboratório do mapa tático V8">
      <header className={styles.header}>
        <div>
          <span className={styles.eyebrow}>Somente ADM · motor tático V8</span>
          <h1>Laboratório do Mapa Tático</h1>
          <p>Skills, recursos, reações, Doutrinas, alcance, controles e operações espaciais.</p>
        </div>
        <div className={styles.status}><small>Funcional</small><strong>V8 · Rodada {round}</strong></div>
      </header>
