...
export function ClassCodex({ entries }: { entries: Entry[] }) {
  ...
  return <div className="lore-browser">
    <aside className="lore-browser__index"><label><span>Buscar classe</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Nome ou função"/></label><nav>{visible.map((item) => <button className={item.slug === entry.slug ? "is-active" : ""} onClick={() => setSelected(item.slug)} key={item.slug}><i>{item.name.slice(0,2).toUpperCase()}</i><span><strong>{item.name}</strong><small>{item.payload.specialization}</small></span><b>{"★".repeat(item.payload.difficulty)}</b></button>)}</nav></aside>
    <article className="lore-dossier"><header><div className="lore-dossier__sigil">{entry.name.slice(0,2).toUpperCase()}</div><div><span className="eyebrow">Arquivo de classe</span><h2>{entry.name}</h2><p>{entry.payload.description}</p></div><dl><div><dt>Dificuldade</dt><dd>{entry.payload.difficulty}/5</dd></div><div><dt>Função</dt><dd>{entry.payload.specialization}</dd></div><div><dt>Atributos</dt><dd>{entry.payload.primaryAttributes.join(" · ")}</dd></div></dl></header>
