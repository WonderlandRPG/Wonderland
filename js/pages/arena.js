...
(async function(){
  ...
  const state={characters:[],sheet:null,character:null,finalAttrs:null,skills:[]};
  ...
  async function loadCharacters(){
    ...
    let characters=[];
    try{characters=await account.listCharacters()}catch(error){console.error(error)}
    ...
    if(state.characters.length)await loadCharacter(state.characters[0].id)
  }
  ...
  async function loadCharacter(id){
    ...
    try{
      ...
      state.skills=officialSkills(cls,Number(sheet.character.level||1),sheet.character.class_path_id);
      ...
      message.textContent="Combatente pronto.";
  ...
  function runTest(){
    if(!state.finalAttrs){message.textContent="Selecione um personagem.";return}
    const selected=skillSelect.value==="basic"?null:state.skills[Number(skillSelect.value)];
    const parsed=selected?parseSkillScale(textValue(selected.descricao)):null;
    const type=damageType.value;
    const defaultAttr=type==="magical"?"INT":"FOR";
    const scalingAttr=parsed?.attr||defaultAttr;
    const percent=Math.max(0,Number(scaleInput.value)||100);
    const attrValue=Number(state.finalAttrs[scalingAttr]||0);
    const raw=Math.round(attrValue*(percent/100));
    const targetDef=Math.max(0,Number(enemyDef.value)||0);
    const targetRes=Math.max(0,Number(enemyRes.value)||0);
    let reduction=0;
    if(type==="physical")reduction=defense?.breakdown({def:targetDef,res:targetRes})?.physicalReduction??Math.min(75,Math.floor(targetDef/5));
    if(type==="magical")reduction=defense?.breakdown({def:targetDef,res:targetRes})?.magicalReduction??Math.min(75,Math.floor(targetRes/5));
    if(type==="true")reduction=defense?.breakdown({def:targetDef,res:targetRes})?.trueReduction??Math.min(75,Math.floor(targetRes/10));
    const final=Math.max(0,Math.round(raw*(1-reduction/100)));
    const turns=Math.max(1,Math.min(20,Number(turnsInput.value)||1));
    const total=final*turns;
    const hp=Math.max(1,Number(enemyHp.value)||1);
    const remaining=Math.max(0,hp-total);
    resultCards.innerHTML=`<article><span>Dano bruto</span><strong>${raw}</strong></article><article><span>Redução</span><strong>${reduction}%</strong></article><article><span>Dano final</span><strong>${total}</strong></article><article><span>HP restante</span><strong>${remaining}</strong></article>`;
    combatLog.innerHTML=`<p><strong>${esc(state.character.name)}</strong> usa <strong>${esc(selected?textValue(selected.nome):"Ataque básico")}</strong>.</p><p>Escala utilizada: ${percent}% de ${scalingAttr} (${attrValue}) = ${raw} de dano bruto.</p><p>O alvo reduz ${reduction}% por suas defesas. Dano por turno: ${final}.</p><p>${turns} turno(s): ${final} × ${turns} = <strong>${total} de dano final</strong>.</p><p>HP do alvo: ${hp} → ${remaining}.</p>`;
    message.textContent=remaining<=0?"Teste concluído: alvo derrotado.":"Teste concluído.";
  }
  ...
  [enemyDef,enemyRes,enemyHp].forEach(input=>input.addEventListener("input",syncEnemyView));
