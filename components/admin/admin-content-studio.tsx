...
function ClassForm({value,onChange}:{value:SimpleClassDraft;onChange:(v:SimpleClassDraft)=>void}){return <><Text label="Nome" value={value.name} onChange={(name)=>onChange({...value,name})}/><TextArea label="Descrição para jogadores" value={value.description} onChange={(description)=>onChange({...value,description})}/><div className={styles.grid}><Text label="Especialização" value={value.specialization} onChange={(specialization)=>onChange({...value,specialization})}/><Select label="Atributo principal" value={value.primaryAttribute} options={attributes.map(a=>[a,a])} onChange={(v)=>onChange({...value,primaryAttribute:v as SimpleClassDraft["primaryAttribute"]})}/><Number label="Dificuldade (1–5)" value={value.difficulty} min={1} max={5} onChange={(difficulty)=>onChange({...value,difficulty})}/><Text label="Nome do recurso" value={value.resourceName} onChange={(resourceName)=>onChange({...value,resourceName})}/><Number label="Máximo do recurso" value={value.resourceMaximum} min={1} max={999} onChange={(resourceMaximum)=>onChange({...value,resourceMaximum})}/><Text label="Imagem (URL)" value={value.imageUrl} onChange={(imageUrl)=>onChange({...value,imageUrl})}/><Text label="Nome da passiva" value={value.passiveName} onChange={(passiveName)=>onChange({...value,passiveName})}/></div><TextArea label="Descrição da passiva" value={value.passiveDescription} onChange={(passiveDescription)=>onChange({...value,passiveDescription})}/></>}
function RaceForm({value,onChange}:{value:SimpleRaceDraft;onChange:(v:SimpleRaceDraft)=>void}){return <><Text label="Nome" value={value.name} onChange={(name)=>onChange({...value,name})}/><TextArea label="Descrição da raça" value={value.description} onChange={(description)=>onChange({...value,description})}/><div className={styles.grid}><Text label="Especialização" value={value.specialization} onChange={(specialization)=>onChange({...value,specialization})}/><Number label="Dificuldade" value={value.difficulty} min={1} max={5} onChange={(difficulty)=>onChange({...value,difficulty})}/><Number label="HP inicial" value={value.baseHp} min={150} max={800} onChange={(baseHp)=>onChange({...value,baseHp})}/><Number label="Mana inicial" value={value.baseMana} min={0} max={999} onChange={(baseMana)=>onChange({...value,baseMana})}/><Text label="Imagem (URL)" value={value.imageUrl} onChange={(imageUrl)=>onChange({...value,imageUrl})}/><Text label="Nome do traço" value={value.traitName} onChange={(traitName)=>onChange({...value,traitName})}/></div><TextArea label="Descrição do traço racial" value={value.traitDescription} onChange={(traitDescription)=>onChange({...value,traitDescription})}/><Attributes label="Bônus raciais · máximo 25" value={value.bonuses} onChange={(bonuses)=>onChange({...value,bonuses})}/></>}
function ItemForm({value,onChange}:{value:SimpleItemDraft;onChange:(v:SimpleItemDraft)=>void}){return <><Text label="Nome" value={value.name} onChange={(name)=>onChange({...value,name})}/><TextArea label="Descrição" value={value.description} onChange={(description)=>onChange({...value,description})}/><div className={styles.grid}><Text label="Categoria" value={value.category} onChange={(category)=>onChange({...value,category})}/><Select label="Slot" value={value.slot} options={itemSlots.map(v=>[v,slotLabels[v]])} onChange={(v)=>onChange({...value,slot:v as SimpleItemDraft["slot"]})}/><Select label="Raridade" value={value.rarity} options={rarities.map(v=>[v,rarityLabels[v]])} onChange={(v)=>onChange({...value,rarity:v as SimpleItemDraft["rarity"]})}/><Number label="Preço WG" value={value.price} min={0} max={999999999} onChange={(price)=>onChange({...value,price})}/><Text label="Imagem (URL)" value={value.imageUrl} onChange={(imageUrl)=>onChange({...value,imageUrl})}/><label><span>Arma de duas mãos?</span><select value={value.twoHanded?"yes":"no"} onChange={(e)=>onChange({...value,twoHanded:e.target.value==="yes"})}><option value="no">Não</option><option value="yes">Sim</option></select></label></div><Attributes label="Atributos concedidos" value={value.attributes} onChange={(attributes)=>onChange({...value,attributes})}/><EffectFields value={value} onChange={onChange}/></>}
...
function EffectFields<T extends SimpleItemDraft|SimpleTitleDraft>({value,onChange}:{value:T;onChange:(v:T)=>void}){return <><div className={styles.grid}><Select label="Efeito especial" value={value.effectKind} options={effectKinds.map(v=>[v,effectLabels[v]])} onChange={(v)=>onChange({...value,effectKind:v as T["effectKind"]})}/><Text label="Nome do efeito" value={value.effectName} onChange={(effectName)=>onChange({...value,effectName})}/><Number label="Potência" value={value.effectPower} min={0} max={1000} onChange={(effectPower)=>onChange({...value,effectPower})}/><Number label="Duração" value={value.effectDuration} min={0} max={20} onChange={(effectDuration)=>onChange({...value,effectDuration})}/></div><TextArea label="Descrição do efeito" value={value.effectDescription} onChange={(effectDescription)=>onChange({...value,effectDescription})}/></>}
function Attributes({label,value,onChange}:{label:string;value:AttributeDraft;onChange:(v:AttributeDraft)=>void}){return <fieldset className={styles.attributeBox}><legend>{label}</legend><div className={styles.attributeGrid}>{attributes.map((key)=><Number key={key} label={key} value={value[key]} min={0} max={999} onChange={(n)=>onChange({...value,[key]:n})}/>)}</div></fieldset>}
...
function Number({label,value,min,max,onChange}:{label:string;value:number;min:number;max:number;onChange:(v:number)=>void}){return <label><span>{label}</span><input type="number" min={min} max={max} value={value} onChange={(e)=>onChange(Number(e.target.value))}/></label>}
