...
export function PortalHeadlineEditor({ initial }: { initial: { seasonLabel: string; firstLine: string; secondLine: string } }) {
  ...
  return <section className="admin-portal-editor">
    <form action={savePortalHeadlineAction} className="admin-editor-card admin-form">
      <label className="form-field">Texto da temporada<input maxLength={60} name="seasonLabel" onChange={(event)=>setSeasonLabel(event.target.value)} required value={seasonLabel}/></label>
