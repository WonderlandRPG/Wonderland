...
function JrpgBattle({
  ...
  const activeId = turnOrder[turnIndex % Math.max(1, turnOrder.length)];
  ...
  function advanceTurn(nextPlayer = player, nextEnemy = enemy, text = message) {
    const periodicPlayer = resolvePeriodicItemDamage(nextPlayer, (amount, type) =>
      calculateDamage(amount, type, getEffectiveAttributes(nextPlayer), rules),
    );
    const periodicEnemy = resolvePeriodicItemDamage(nextEnemy, (amount, type) =>
      calculateDamage(amount, type, getEffectiveAttributes(nextEnemy), rules),
    );
    const cooledPlayer = tickCooldowns(periodicPlayer.combatant);
    const cooledEnemy = tickCooldowns(periodicEnemy.combatant);
    setPlayer(cooledPlayer);
    setEnemy(cooledEnemy);
    const nextIndex = (turnIndex + 1) % Math.max(1, turnOrder.length);
    if (nextIndex === 0) setRound((value) => value + 1);
    setTurnIndex(nextIndex);
    setPanel("root");
    const periodic = [...periodicPlayer.messages, ...periodicEnemy.messages];
    setMessage(`${text}${periodic.length ? ` ${periodic.join(" ")}` : ""}`);
  }

  function applyPlayerResult(result: { actor: CombatantState; target: CombatantState; event: { kind: string; message: string } }) {
    ...
    advanceTurn(result.actor, result.target, result.event.message);
  }
  ...
  function useSkill(kind: "class" | "race", key: string) {
    ...
    const result =
      skill.area > 0
        ? (() => {
            const area = resolveAreaSkill(player, [enemy], skill, rules);
            return { actor: area.actor, target: area.targets[0], event: area.events[0] };
          })()
        : kind === "race"
          ? resolveRaceAbility(player, enemy, skill, rules)
  ...
  function useItem(id: string) {
    ...
    advanceTurn({ ...player, hp: player.hp + healed }, enemy, `${character.name} usou ${item.name} e recuperou ${healed} de HP.`);
  }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      advanceTurn(result.target, result.actor, result.event.message);
    }, 450);
    ...
    <section className="arena-console jrpg-battle">
      <header className="arena-toolbar arena-game-header">
        <div>
          ...
          <p>Uma ação por turno. INI define quem age primeiro.</p>
        </div>
      ...
      <div className="jrpg-turn-order">
        {turnOrder.map((id, index) => (
          <span className={activeId === id ? "is-active" : ""} key={id}>{index + 1}. {id === player.id ? player.name : enemy.name}</span>
        ))}
      </div>
      ...
      {!finished ? (
        <section className="arena-command-panel jrpg-command-panel">
          <header><div><span className="eyebrow">Comandos</span><h2>{playerTurn ? "Escolha uma ação" : `Turno de ${enemy.name}`}</h2></div><small>Cada comando encerra o turno.</small></header>
          {panel === "root" ? (
            <div className="pvp-actions-grid jrpg-actions">
              <Command disabled={!playerTurn} name="Atacar" detail="Ataque básico" onClick={() => applyPlayerResult(resolveBasicAttack(player, enemy, rules))} />
              <Command disabled={!playerTurn || character.skills.length === 0} name="Habilidades" detail={`${character.skills.length} de classe`} onClick={() => setPanel("class")} />
              <Command disabled={!playerTurn || character.raceAbilities.length === 0} name="Raça" detail={`${character.raceAbilities.length} racial(is)`} onClick={() => setPanel("race")} />
              <Command disabled={!playerTurn || character.items.length === 0} name="Item" detail={`${character.items.length} disponível(is)`} onClick={() => setPanel("item")} />
              <Command disabled={!playerTurn || (player.cooldowns["defesa-total"] ?? 0) > 0} name="Defender" detail="Bloqueia o próximo dano" onClick={() => advanceTurn(guardCombatant(player), enemy, `${player.name} assumiu Defesa Total.`)} />
              <Command disabled={!playerTurn} name="Aguardar" detail="Passa o turno" onClick={() => advanceTurn(player, enemy, `${player.name} aguardou.`)} />
            </div>
          ) : (
        ...
        <section className="arena-result">
          ...
          {mode === "pve" && enemy.hp <= 0 ? (
            <>
              ...
              {!reward ? <button className="button button--primary" disabled={claiming} onClick={claimReward} type="button">{claiming ? "Registrando…" : "Receber recompensa"}</button> : <strong>+{reward.xp.toLocaleString("pt-BR")} XP · +{reward.wg.toLocaleString("pt-BR")} WG recebidos</strong>}
...
function CombatantCard({ fighter, name, subtitle, imageUrl, active }: { fighter: CombatantState; name: string; subtitle: string; imageUrl: string; active: boolean }) {
  const resource = fighter.maxClassResource > 0 ? `${fighter.classResourceName}: ${fighter.classResource}/${fighter.maxClassResource}` : fighter.maxMana > 0 ? `Mana: ${fighter.mana}/${fighter.maxMana}` : null;
  return <article className={`pvp-fighter jrpg-fighter ${active ? "is-active" : ""}`}><div className="jrpg-fighter__portrait" style={imageUrl ? { backgroundImage: `url(${imageUrl})` } : undefined}>{!imageUrl ? name.slice(0, 2).toUpperCase() : null}</div><h3>{name}</h3><p>{subtitle}</p><progress max={fighter.maxHp} value={fighter.hp} /><strong>{fighter.hp.toLocaleString("pt-BR")} / {fighter.maxHp.toLocaleString("pt-BR")} HP</strong>{resource ? <em>{resource}</em> : null}</article>;
}
