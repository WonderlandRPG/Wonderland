    ...
    <div className={styles.book}>
      ...
      <article className={styles.sheet}>
        ...
        <section className={styles.section}>
          <div className={styles.mechanic}><div><small>Recurso exclusivo</small><h3>{entry.payload.resource.name}</h3><strong>{entry.payload.resource.initial} inicial · máximo {entry.payload.resource.maximum}</strong></div><p>{entry.payload.mechanic.description}</p></div>
        </section>

        <section className={styles.section}>
          <div className={styles.sectionTitle}><span>I</span><div><small>Nível 50</small><h3>Caminhos de {entry.name}</h3></div></div>
          <div className={styles.pathGrid}>
            {entry.payload.paths.map((path) => (
              <article className={styles.path} key={path.key}>
                <small>Desbloqueio no nível {path.unlockLevel}</small><h4>{path.name}</h4><p>{path.description}</p>
                <div className={styles.pathMeta}><small>Missão de escolha</small><h4>{path.quest.title}</h4><p>{path.quest.briefing}</p><ol>{path.quest.objectives.map((objective) => <li key={objective}>{objective}</li>)}</ol></div>
                <div className={styles.pathMeta}><small>Passiva</small><h4>{path.passive.name}</h4><p>{path.passive.description}</p></div>
                {path.skills.map((skill) => <div className={styles.pathMeta} key={skill.key}><small>Nível {skill.level} · {skill.category}</small><h4>{skill.name}</h4><p>{skill.playerDescription}</p></div>)}
              </article>
            ))}
          </div>
        </section>
        ...
        <section className={styles.section}>
          <div className={styles.sectionTitle}><span>II</span><div><small>Progressão principal</small><h3>Habilidades da classe</h3></div></div>
          <div className={styles.ledger}>
            {entry.payload.progression.map((skill) => (
              <article className={styles.skill} key={skill.key}>
                ...
                <div className={styles.skillBody}><header><b>{skill.name}</b><em>{skill.type} · {skill.category}</em></header><p>{skill.playerDescription}</p><footer><span>{skill.cost ? `${skill.cost} ${entry.payload.resource.name}` : "Sem custo"}</span><span>Recarga {skill.cooldown}</span><span>{skill.reachText}</span></footer></div>
              </article>
            ))}
          </div>
        </section>
