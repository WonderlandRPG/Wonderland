...
describe("simple admin skill builder", () => {
  it("builds a single target damage skill", () => {
    const skill = buildClassSkillFromSimpleDraft({
      ...simpleDraftDefaults(),
      name: "Golpe Brutal",
      description: "Atinge um inimigo com um golpe pesado.",
      multiplier: 2,
      attribute: "FOR",
    });
  ...
  it("builds an area heal capped by the selected target count", () => {
    const skill = buildClassSkillFromSimpleDraft({
      ...
      description: "Restaura a vida de até três aliados escolhidos.",
      effectType: "heal",
      targetSide: "ally",
      targetCount: 3,
      attribute: "INT",
  ...
  it("turns debuff values negative in the engine contract", () => {
    const skill = buildClassSkillFromSimpleDraft({
      ...simpleDraftDefaults(),
      name: "Quebra-Guarda",
      description: "Enfraquece a defesa do inimigo por dois turnos.",
      effectType: "debuff",
      targetSide: "enemy",
      modifierAttribute: "DEF",
      modifierValue: 20,
      duration: 2,
      damageType: "none",
    });
