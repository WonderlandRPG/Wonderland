...
export const simpleSkillDraftSchema = z.object({
  ...
  attribute: z.enum(["FOR", "DEF", "RES", "INI", "INT"]).default("FOR"),
  ...
  modifierAttribute: z.enum(["FOR", "DEF", "RES", "INI", "INT"]).default("FOR"),
...
export function buildClassSkillFromSimpleDraft(input: SimpleSkillDraft): ClassSkill {
  ...
  const scaling =
    draft.multiplier > 0
      ? [{ attribute: draft.attribute as AttributeKey, multiplier: draft.multiplier }]
      : [];
  const operationScaling = scaling;
  const status =
    draft.statusName ||
    (draft.effectType === "buff"
      ? "fortalecido"
      : draft.effectType === "debuff"
        ? "enfraquecido"
        : draft.effectType === "stun"
          ? "atordoado"
          : "");
  const operation: ClassSkill["operations"][number] = {
    operation:
      draft.effectType === "damage"
        ? "DAMAGE"
        : draft.effectType === "heal"
          ? "HEAL"
          : draft.effectType === "shield"
            ? "SHIELD"
            : draft.effectType === "buff"
              ? "BUFF"
              : draft.effectType === "debuff"
    ...
    status,
    duration:
      draft.effectType === "damage" || draft.effectType === "heal" || draft.effectType === "shield"
        ? 0
        : Math.max(1, draft.duration),
    chance: draft.chance,
    stacks: 1,
    maxStacks: 1,
    distance: 0,
    modifiers:
      draft.effectType === "buff"
        ? [{ attribute: draft.modifierAttribute, value: draft.modifierValue }]
        : draft.effectType === "debuff"
          ? [{ attribute: draft.modifierAttribute, value: -draft.modifierValue }]
          : [],
  };
  const targetText =
    draft.targetSide === "self" ? "si mesmo" : draft.targetSide === "ally" ? "aliado" : "inimigo";
  const amountText = isArea ? `${draft.targetCount} alvos` : `1 ${targetText}`;
  const multiplierText =
    draft.multiplier > 0 ? `${draft.multiplier}x ${draft.attribute}` : "sem multiplicador";
  const skill: ClassSkill = {
    key: slug(draft.name),
    name: draft.name,
    level: draft.level,
    category: category(draft.effectType),
    type: "Ativa",
    effect: draft.description,
    kind: kind(draft.effectType),
    damageType: draft.effectType === "damage" ? draft.damageType : "none",
    target: isArea ? "area" : draft.targetSide,
    resource: draft.resource,
    resourceKey: draft.resourceKey,
    cost: draft.cost,
    cooldown: draft.cooldown,
    range: 0,
    area: isArea ? draft.targetCount : 0,
    duration: draft.duration,
    scaling,
    reachText: isArea
      ? `${draft.targetCount} alvos selecionáveis`
      : `Alvo selecionado: ${targetText}`,
    conditions: [],
    systemRule: `Executa ${operation.operation} em ${amountText}. Base ${draft.baseValue}; escala ${multiplierText}; chance ${draft.chance}%.`,
    playerDescription: draft.description,
    chance: draft.chance,
    maxStacks: 1,
...
export function simpleDraftDefaults(): SimpleSkillDraft {
  return {
    ...
    description: "Descreva o que a habilidade faz para o jogador.",
    level: 1,
    effectType: "damage",
    targetSide: "enemy",
    targetCount: 1,
